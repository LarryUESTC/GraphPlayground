...
import copy
import random
# conn = psycopg2.connect(host="121.48.165.6", port="40201",
#                         database="gan", user="kongfei", password="262300aa")
# cursor = conn.cursor()
...
def get_columns(train_val_metrics, val_metrics, test_val_metrics, type_dict=None):
    ...
    for name in val_metrics:
        columns['val' + name] = "double precision" if name not in type_dict else type_dict[name]
...
if __name__ == "__main__":
    writer = WriteToDatabase({'host': "121.48.165.6", "port": "40201",
                              "database": "gan", "user": "kongfei", "password": "262300aa"},
                             "log",
                             {"test1": 'int', "test3": "int", "test2": "int"},
                             {"test4": "int"},
                             {"test1": 0, 'test2': 0},
                             {"test1": 0, 'test2': 0}, True, True)
    writer.init()
    writer.write({'test3': 10}, {"test4": 5})
    writer.write({'test3': 11}, {"test4": 6})
    writer.write({'test3': 10}, {"test4": 12})
    writer.close()
